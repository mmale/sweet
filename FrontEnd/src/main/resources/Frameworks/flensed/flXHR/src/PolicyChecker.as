/*	flXHR 0.6 alpha7 (PolicyChecker) <http://www.flxhr.com/>
...
/* Public */
	...
	public function PolicyChecker(rootObj:DisplayObject) {
		...
		if (pageloc != null) {
			_page_url = pageloc; 
			_page_url = __url_wipe_case(_page_url);
	...
	public function CheckHeaderPolicies(targetLocation:String,headerName:String):Boolean {
		...
		if (PolicyCheckNeeded()) {
			...
			headerAllowed = (__check_header_domain(_policy_definitions[tunique]["from"],targetpath,headerName) &&
								__check_swf_domain(_policy_definitions[tunique]["from"],targetpath));
		}
...
/* Private */
	...
	private function __glue_url(urlparts:Object):String {
		return (urlparts.protocol+"://"+((urlparts.userInfo!="")?urlparts.userInfo+"@":"")+urlparts.host+((urlparts.port!="")?":"+urlparts.port:"")+((urlparts.relative.charAt("/")<0)?"/":"")+urlparts.relative);
	}
	...
	private function __qualified_url(url:String):String {
		...
		if (sepPosition < 0 || sepPosition > 5 && _page_url != "") {	// doesn't have a valid "protocol" specifier at the beginning,
													// so assume this is a relative URL and use the page's URL to
													// fully-qualify the passed in URL
			...
			return url = pageurlparts.protocol+"://"+unpw+pageurlparts.host+port+((url.charAt(0)!="/")?"/":"")+url;
		}
		else if (url != "") {
	...
	private function __domain_parts(domain:String):Array {
		...
		if (parts.length > 1) {
			var tld:String = parts[parts.length-1];
			parts.pop();
			if (parts.length > 1 || parts[0] != "*") parts[parts.length-1] += "."+tld;
			else parts[0] = tld;
		}
		return parts;
	}
	...
	private function __insert_allow_to(toNode:Object,path:String,secure:Boolean):void {
		...
		while (cardinality<=20) {	// no paths deeper than 20 levels, to limit memory and recursion performance issues
	...
	private function __insert_allow_from(fromNode:Object,domain:String,path:String,secure:Boolean):void {
		...
		while (cardinality<=10) {	// no sub-domains deeper than 10 levels, to limit memory and recursion performance issues
	...
	private function __insert_allow_header_from(fromNode:Object,domain:String,path:String,header:String,secure:Boolean):void {
		if (domain=="" || domain=="*" || domain.indexOf("*")>0 || (domain.indexOf("*")==0 && domain.indexOf(".")>=0 && domain.split(".").length<2)) return;
		if (header=="" || header.split("*").length > 2 || (header.indexOf("*")>0 && header.indexOf("*")<(header.length-1))) return;
		var cardinality:uint = 1;
		var theNode:Object = fromNode;
		var dparts:Array = __domain_parts(domain);
	...
	private function __check_header_domain(fromNode:Object,path:String,header:String):Boolean {
		for (var j in fromNode) {
			if (__domain_match(j,_page_domain)) {
				if (__check_header_path(fromNode[j]["to"],path,header)) return true;
				if (__check_header_domain(fromNode[j]["from"],path,header)) return true;
			}
		}
		return false;
	}
	...
	private function __check_header_path(toNode:Object,path:String,header:String):Boolean {
		for (var j in toNode) {
			if (__path_match(path,j)) {
				if (typeof toNode[j]["headers"][header]!="undefined" && (!toNode[j]["headers"][header]["secure"] || (ParseUri(_page_url).protocol == "https" && ParseUri(_swf_url).protocol == "https"))) return true;
								...
								if (policyentries.name() == "cross-domain-policy") {
									...
									if (elementlist.length() > 0) {
										for (j in elementlist) {
											attrval = elementlist[j].attribute("domain");
											if (attrval.indexOf("*") <= 0) {
												...
												if (attrval != "*") __insert_allow_from(_policy_definitions[targetunique]["from"]["*"]["from"],attrval,toPath,(attrval2==="true"));
												else __insert_allow_to(_policy_definitions[targetunique]["from"]["*"]["to"],toPath,(attrval2==="true"));
											}
										}
										...
										if (elementlist.length() > 0) {
											for (j in elementlist) {
												try {
													...
													for (l in headers) {
														if (attrval != "*") __insert_allow_header_from(_policy_definitions[targetunique]["from"]["*"]["from"],attrval,toPath,headers[l].toLowerCase(),(attrval2==="true"));
														else __insert_allow_header_to(_policy_definitions[targetunique]["from"]["*"]["to"],toPath,headers[l].toLowerCase(),(attrval2==="true"));
													}
												} catch (err) { }
											}
										}
