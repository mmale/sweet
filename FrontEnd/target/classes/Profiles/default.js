(function(){
	...
	var namespace 	  = 'http://purl.oclc.org/NET/sweetserver';
...
$("*").hover(
	function (event) {
		...
		$(this).bind('click', function(event) {
			...
			var seeAlso = 'http://purl.oclc.org/NET/sweetserver/store/' + (new UUID() + '').toLowerCase();
			...
			RDFA.CALLBACK_DONE_PARSING = function() {
				...
				$('body').append('<div id="temp" style="xdisplay: none"><form id="fnt" method="POST" action="http://sweetdemo.kmi.open.ac.uk/store" target="iii"><textarea id="nt" name="nt" cols="100" rows="10"></textarea><input type="submit"></form><iframe src="about:blank" name="iii" id="iii"></iframe></div>');
				$('#nt').text(nt);
				//$('#fnt').submit();
				//$('#temp').remove();
...
/////////							
			...
			session: 	'http://purl.oclc.org/NET/sweetserver/store/' 
						+ (new UUID() 
						+ '').toLowerCase()  
		});
